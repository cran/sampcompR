...
#' ##################################################
#' 
#' reduce_df_ols3 <- function (df,dependent,independent, rm_na="pairwise", 
...
#' 
#' 
#'   if (is.null(weight_var)& is.null(raking_vars)==F) df<-subset(df, select=c(variables,raking_vars))
#'   if (is.null(weight_var)& is.null(raking_vars)==T) df<-subset(df, select=c(variables))
#'   
#'   if (is.null(weight_var)==F & is.null(raking_vars)==F) df<-subset(df, select=c(variables,weighting,raking_vars))
#'   if (is.null(weight_var)==F & is.null(raking_vars)==T) df<-subset(df, select=c(variables,weighting))
#'   #reduce df to dependent or independent variables
#'   if(rm_na=="listwise") df<-stats::na.omit(df)
#' 
#'   #reduce df to the model
#'   if(is.null(raking_vars)==T){
...
#'     df_list[[i]]<-dataframe}}
#'   
#'   if(is.null(raking_vars)==F){
#'   for (i in 1:length(dependent)){
#'     if (is.null(weight_var)) dataframe<-subset(df,select = c(dependent[i],independent,raking_vars))
#'     if (is.null(weight_var)==F) dataframe<-subset(df,select = c(dependent[i],independent, weighting,raking_vars))
#'     if (is.null(weight_var)==F) names(dataframe)[names(dataframe)==weight_var]<-"df_weights"
#'     if (is.null(weight_var)==F & is.null(id)==F) names(dataframe)[names(dataframe)==id]<-"id_df"
#'     if (is.null(weight_var)==F & is.null(strata)==F) names(dataframe)[names(dataframe)==strata]<-"strata_df"
#'     if (rm_na=="pairwise") dataframe<-stats::na.omit(dataframe)
...
#' ######################################################
#' 
#' combine_dfs3<-function(df,benchmark,dependent,independent,
...
#' 
#'     if(is.null(raking_vars)==F){
#'       for (r in 1:length(raking_vars)){
#'         dataframe2[raking_vars[r]]<-NA
#'       }
#'     }
#'     
...
#' weighted_design_ols<-function(data_list,dependent, weight_var, 
#'                               id, strata=NULL, nest=FALSE, type="ineract", 
#'                               raking_vars=NULL, raking_targets=NULL,
#'                               raked_design=NULL){
#'   design_list<-list()
#'   for (i in 1:length (dependent)){
#'     if(type=="interact"){
#'       
#'       if(is.null(raked_design)==F){
#'         
#'         raked_weight<-weights(raked_design[[i]])
...
#'         
#' 
#'         rake_margins<-purrr::map(paste0("~",raking_vars),as.formula)
#'         
...
#' 
#' ### weight every model ###
#' weighted_glm_ols3 <- function(df, formula, design=NULL, method="ols", 
#'                               raking_vars=NULL,
#'                               raking_targets=NULL,
#'                               design_list_df=NULL) {
#'   
#'   # if(is.null(design_list_df)==F & is.null(raking_vars)==F){
#'   #   
#'   #   raking_vars<-purrr::map(paste0("~",raking_vars),as.formula)
#'   #   #return(raking_targets)
#'   #   survey_design_raked <- rake(design = design_list_df, 
#'   #                               population.margins = raking_targets, 
#'   #                               sample.margins = raking_vars, 
...
#' #############################################################
#' 
#' run_ols3<-function(df_comb,dependent,independent, design_list=NULL, 
#'                    method="ols", type="interact",
#'                    raking_vars=NULL,
#'                    raking_targets=NULL,
#'                    design_list_df=NULL){
...
#'         if(is.null(design_list)==F) ols_next<- weighted_glm_ols3 (comb_data, design=design_list[[i]], formula= form, design_list_df=design_list_df[[i]])}
#' 
#'       if(type=="df1"){
...
#'         if(is.null(design_list)==F) ols_next<- weighted_glm_ols3 (comb_data, design=design_list[[i]], formula= form, method= "logit", design_list_df=design_list_df[[i]])}
#' 
#'       if(type=="df1"){
...
#'         if(is.null(design_list)==F) ols_next<- weighted_glm_ols3 (comb_data[comb_data$sample_ident==0,], design=design_list[[i]], formula= form, method= "logit", design_list_df=design_list_df[[i]])}
#' 
#'       if(type=="bench"){
...
#'         if(is.null(design_list)==F) ols_next<- weighted_glm_ols3 (comb_data[comb_data$sample_ident==1,], design=design_list[[i]], formula= form, method= "logit", design_list_df=design_list_df[[i]])}
#'     }
#' 
#'     ols_list[[i]]<-ols_next
...
#' 
#' 
#' final_ols_list3<-function(ols_list, dependent,independent,weight_var=NULL,robust_se=F, 
...
#'   if(nboots>1) {
#'     
#'     p_se_list<-multi_boot3(df=df,benchmark=benchmark,dependent,independent,id = id,
...
#'                      raking_vars = raking_vars, raking_targets = raking_targets)
#'     
...
#' 
#' 
#' multi_ols_compare3<-function(df,benchmark,independent,dependent,rm_na="pairwise", out_olslist=T,
...
#'                             nboots=0, parallel=F, raking_vars=NULL,
#'                             raking_targets=NULL){
#' 
...
#'   
#'   if (is.null(weight)) df<-reduce_df_ols3(df, dependent, independent, rm_na = rm_na, raking_vars = raking_vars)
#'   if (is.null(weight_bench)) benchmark<-reduce_df_ols3(benchmark, dependent, independent, rm_na = rm_na)
#'   if (is.null(weight)==F) df<-reduce_df_ols3(df, dependent, independent,  weight_var = weight, id = id, rm_na = rm_na, raking_vars = raking_vars)
#'   if (is.null(weight_bench)==F) benchmark<-reduce_df_ols3(benchmark, dependent, independent,  weight_var = weight_bench, id = id_bench, rm_na = rm_na)
#' 
#' 
#'   df_comb<-combine_dfs3(df,benchmark,dependent,independent,id=id,id_bench=id_bench,
#'                        weight=weight,weight_bench=weight_bench,
#'                        strata=strata,strata_bench=strata_bench, 
#'                        raking_vars = raking_vars)
#' 
#'   # df_comb2<-combine_dfs3(benchmark,df,dependent,independent,id=id_bench,id_bench=id,
...
#'   if (is.null(weight)==F) {
#'     design_list<-list()
#'     if(is.null(raking_vars)==T) design_list[[2]] <- weighted_design_ols(df_comb,dependent,weight_var="df_weights", id="id_df", strata=NULL, nest=F, type="df1")
#'     if(is.null(raking_vars)==F) design_list[[2]] <- weighted_design_ols(df_comb,dependent,weight_var="df_weights", id="id_df", strata=NULL, nest=F, type="df1",
#'                                                                         raking_vars = raking_vars, raking_targets=raking_targets)
#'     if(is.null(raking_vars)==T) design_list[[1]] <- weighted_design_ols(df_comb,dependent,weight_var="df_weights", id="id_df", strata=NULL, nest=F, type="interact")
#'     if(is.null(raking_vars)==F) design_list[[1]] <- weighted_design_ols(df_comb,dependent,weight_var="df_weights", id="id_df", strata=NULL, nest=F, type="interact", 
...
#' 
#'   } else {design_list = design_list[[3]]}
#'   
#'   if (is.null(weight)==T & is.null(raking_vars)==F) {
#'     design_list[[2]] <- weighted_design_ols(df_comb,dependent,weight_var="df_weights", id="id_df", strata=NULL, nest=F, type="rake_df1",
#'                                             raking_vars = raking_vars, raking_targets=raking_targets)
#'     
#'     
#'     design_list[[1]] <- weighted_design_ols(df_comb,dependent,weight_var="df_weights", id="id_df", strata=NULL, nest=F, type="rake_interact", 
#'                                             raked_design = design_list[[2]])
#'   }
#' 
...
#'   if (method=="ols") {
#'   ols_list<-list()
#'    ols_list[[1]]<-run_ols3(df_comb = df_comb,dependent,independent, design_list =  design_list[[1]], type="interact",design_list_df =design_list[[2]])
#'    ols_list[[2]]<-run_ols3(df_comb = df_comb,dependent,independent, design_list =  design_list[[2]], type="df1")
#'    ols_list[[3]]<-run_ols3(df_comb = df_comb,dependent,independent, design_list =  design_list[[3]], type="bench")
#' 
#'    # ols_list2<-list()
...
#'   if (method=="logit"){
#'     ols_list<-list()
#'     ols_list[[1]]<-run_ols3(df_comb = df_comb,dependent,independent, design_list =  design_list[[1]], method = "logit", type="interact",design_list_df =design_list[[2]])
#'     ols_list[[2]]<-run_ols3(df_comb = df_comb,dependent,independent, design_list =  design_list[[2]], method = "logit", type="df1")
#'     ols_list[[3]]<-run_ols3(df_comb = df_comb,dependent,independent, design_list =  design_list[[3]], method = "logit", type="bench")
#' 
#'     # ols_list2<-list()
...
#'   output<-final_ols_list3(ols_list, dependent = dependent,
...
#' 
#' 
#' multi_compare3<-function(df,benchmark,independent,dependent, method,rm_na="pairwise", out_output_list=T,
#'                         out_df=F, out_models=F, print_p=F, print_se=F, weight=NULL, id=NULL,
...
#'                         silence_summary=F, nboots=0, parallel = F, raking_vars=NULL,
#'                         raking_targets=NULL){
#' 
#' 
...
#' 
#'   if(method=="ols") output<-multi_ols_compare3(df=df,benchmark=benchmark,independent=independent,
...
#'                                               parallel = parallel,raking_vars=raking_vars,
#'                                               raking_targets=raking_targets)
#' 
#'   if(method=="logit") output<-multi_ols_compare3(df=df,benchmark=benchmark,independent=independent,
#'                                               dependent=dependent,rm_na=rm_na, out_olslist=out_output_list,
#'                                               out_df=out_df,out_models = out_models, print_p=print_p, print_se=print_se,
#'                                               weight=weight, id=id, strata=strata,
#'                                               nest=nest, weight_bench=weight_bench, id_bench=id_bench,
...
#' 
#' 
#' multi_boot_sub3<-function(df,i=NULL,benchmark,dependent,independent,ids = NULL,
#'                          id_bench = NULL,weight_df = NULL,weight_bench = NULL,
#'                          stratas = NULL, strata_bench = NULL, rm_na = "pairwise",
#'                          method = "ols", bootstrap=F, raking_vars=NULL,
#'                          raking_targets=NULL){
#'   
#'   if (bootstrap==T) df<-df[i,]
#'   
#'   
#'   if (is.null(weight_df)) df<-reduce_df_ols3(df, dependent, independent, rm_na = rm_na, raking_vars=raking_vars)
#'   if (is.null(weight_bench)) benchmark<-reduce_df_ols3(benchmark, dependent, independent, rm_na = rm_na)
#'   if (is.null(weight_df)==F) df<-reduce_df_ols3(df, dependent, independent,  weight_var = weight_df, id = ids, rm_na = rm_na,raking_vars=raking_vars)
#'   if (is.null(weight_bench)==F) benchmark<-reduce_df_ols3(benchmark, dependent, independent,  weight_var = weight_bench, id = id_bench, rm_na = rm_na)
#'   
#'   
#'   
#'   df_comb<-combine_dfs3(df,benchmark,dependent,independent,id=ids,id_bench=id_bench,
...
#'   
#'   if(is.null(weight_df)==T  & is.null(raking_vars)==F){
#'     ### fix for a random set of ids ###
#'     design_list[[2]] <- weighted_design_ols(df_comb,dependent,weight_var=NULL, id=NULL, strata=strata, nest=F, type="rake_df1", 
#'                                             raking_vars = raking_vars,raking_targets = raking_targets)
#'     design_list[[1]] <- weighted_design_ols(df_comb,dependent,weight_var=NULL, id=NULL, strata=strata, nest=F, type="rake_interact", 
#'                                             raked_design = design_list[[2]])
#'     
#'     
#'     
...
#' }
#' 
#' multi_boot3<-function(df,benchmark,dependent,independent,id = NULL,
...
#'   
#'   boot_out<-boot(df,statistic = multi_boot_sub3, R = nboots, benchmark = benchmark, 
